  ...
  implements ICategoriaController
{
  ...
  constructor(_categoriaService: ICategoriaService, _validator: IValidator) {
    super();
  ...
  createNewCategoria = async (
    ...
    try {
      ...
      const categoria = await this._categoriaService.find({
        nombre_corto: createDto.nombre_corto,
      });
      if (categoria.length > 0)
        throw new RepeatedNombreCortoError(createDto.nombre_corto);
      const new_categoria_data = await this._categoriaService.createCategoria(
        createDto
      );
      return this.ok(res, {
        id: new_categoria_data.id,
        categoria: new_categoria_data,
      });
    } catch (error) {
