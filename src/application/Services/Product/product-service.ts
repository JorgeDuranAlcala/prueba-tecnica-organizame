...
export class ProductService implements IProductService {
  ...
  async search(query: string) {
    return await SearchProductsUseCase.create(this._productRepository).search(
      query
    );
  }

  async find(props: Record<string | number, any>) {
    return await FindProductUseCase.create(this._productRepository).find(props);
  }

  async updateProduct(id: string, updateProductDto: IUpdateProductDto) {
    return await UpdateProductUseCase.create(this._productRepository).update(
      id,
      updateProductDto
    );
  }
  ...
  async getById(id: string) {
    return await GetProductByIdUseCase.create(this._productRepository).get(id);
  }

  async remove(id: string): Promise<boolean> {
    return await RemoveProductUseCase.create(this._productRepository).remove(
      id
    );
  }
}
